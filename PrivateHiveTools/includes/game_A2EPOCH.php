...
function epochKey($value = '0')
{
	$keys = array('ItemKeyGreen' => range(0, 2500), 'ItemKeyRed' => range(2500, 5000), 'ItemKeyBlue' => range(5000, 7500), 'ItemKeyYellow' => range(7500, 10000), 'ItemKeyBlack' => range(10000, 12500));
	if ($value !== '0') {
		foreach ($keys as $keyname => $keyrange) {
			foreach ($keyrange as $a => $b) {
				if (!is_numeric($value)) {
					if ($value == $keyname . $a) {
						return "$b";
					}
		...
		if ($DATA !== false) {
			...
			if (isset($DATA['INVENTORY[1]'], $DATA['INVENTORY[0]'])) {
				if (is_array($DATA['INVENTORY[1]']) && is_array($DATA['INVENTORY[0]'])) {
					...
					foreach ($DATA['INVENTORY[0]'] as $inv0) {
						if (!empty($inv0)) {
							$inv0 = preg_replace('/[^a-zA-Z0-9_]/', null, $inv0);
							$inventory[0][] = $inv0;
						}
					}
					if (empty($inv1)) {
					...
					foreach ($DATA['INVENTORY[1]'] as $inv1) {
						if (!empty($inv1)) {
							$inv1 = preg_replace('/[^a-zA-Z0-9_]/', null, $inv1);
							$inventory[1][] = $inv1;
						}
					}
			...
			if (isset($DATA['BACKPACK[0]'])) {
				if (!empty($DATA['BACKPACK[0]'])) {
					$backpack[0] = trim($DATA['BACKPACK[0]']);
				}
			}
			if (isset($DATA['BACKPACK[1][0]']) && !empty($DATA['BACKPACK[1][1]'])) {
				if (is_array($DATA['BACKPACK[1][0]'])) {
					foreach ($DATA['BACKPACK[1][0]'] as $back10) {
						if (!empty($back10)) {
							$backpack[1][0][] = preg_replace('/[^a-zA-Z0-9_]/', null, $back10);
						} else {
							$backpack[1][0] = [];
						}
					}
			...
			if (isset($DATA['BACKPACK[1][1]']) && !empty($DATA['BACKPACK[1][0]'])) {
				if (is_array($DATA['BACKPACK[1][1]'])) {
					foreach ($DATA['BACKPACK[1][1]'] as $back11) {
						if (!empty($back11)) {
							$backpack[1][1][] = (int)preg_replace('/[^0-9_]/', null, $back11);
						} else {
							$backpack[1][1] = [];
						}
					}
			...
			if (isset($DATA['BACKPACK[2][0]']) && !empty($DATA['BACKPACK[2][1]'])) {
				if (is_array($DATA['BACKPACK[2][0]'])) {
					foreach ($DATA['BACKPACK[2][0]'] as $back20) {
						if (!empty($back20)) {
							$backpack[2][0][] = preg_replace('/[^a-zA-Z0-9_]/', null, $back20);
						} else {
							$backpack[2][0] = [];
						}
					}
			...
			if (isset($DATA['BACKPACK[2][1]']) && !empty($DATA['BACKPACK[2][0]'])) {
				if (is_array($DATA['BACKPACK[2][1]'])) {
					foreach ($DATA['BACKPACK[2][1]'] as $back21) {
						if (!empty($back21)) {
							$backpack[2][1][] = (int)preg_replace('/[^0-9_]/', null, $back21);
						} else {
							$backpack[2][1] = [];
						}
					}
	...
	if (!empty($VALUE)) {
		...
		if ($row !== false) {
			...
			if ($data['INVENTORY'] !== false) {
				...
				for ($i = 0; $i <= 19; $i++) {
					$items[$i] = (!isset($data['INVENTORY'][1][$i])) ? '' : $data['INVENTORY'][1][$i];
					$items[$i] = (is_array($items[$i])) ? $items[$i][0] : $items[$i];
					$inv1 .= str_replace('{$ITEMS}', $items[$i], $invPut1);
				}
				for ($i = 0; $i <= 15; $i++) {
					$tools[$i] = (!isset($data['INVENTORY'][0][$i])) ? '' : $data['INVENTORY'][0][$i];
					$inv0 .= str_replace('{$TOOLS}', $tools[$i], $invPut0);
				}
			}
			if (!$inv1) {
				for ($i = 0; $i <= 19; $i++) {
					$inv1 .= str_replace('{$ITEMS}', '', $invPut1);
				}
			}
			if (!$inv0) {
				for ($i = 0; $i <= 15; $i++) {
					$inv0 .= str_replace('{$TOOLS}', '', $invPut0);
				}
			}

			if ($data['BACKPACK'] !== false && !empty($data['BACKPACK']) && is_array($data['BACKPACK'])) {
				$back0 = $data['BACKPACK'][0];
				if (!empty($data['BACKPACK'][2]) && !empty($data['BACKPACK'][2][0])) {
					foreach ($data['BACKPACK'][2][0] as $key => $val) {
						$back2 .= str_replace(array('{$ITEMS0}', '{$ITEMS1}'), array($data['BACKPACK'][2][0][$key], $data['BACKPACK'][2][1][$key]), $backPut2);
					}
				}
				if (!empty($data['BACKPACK'][1]) && !empty($data['BACKPACK'][1][0])) {
					foreach ($data['BACKPACK'][1][0] as $key => $val) {
						$back1 .= str_replace(array('{$ITEMS0}', '{$ITEMS1}'), array($data['BACKPACK'][1][0][$key], $data['BACKPACK'][1][1][$key]), $backPut1);
					}
				}
			}
...
function truncObjects()
{
	sqlQuery('TRUNCATE `Object_DATA`;', 'update');
}
function delObjectDamage()
{
	sqlQuery('DELETE FROM `Object_DATA` WHERE `Damage`=1;', 'update');
}
...
function delObject7()
{
	sqlQuery('DELETE FROM `Object_DATA` WHERE datediff( NOW(), `LastUpdated` ) > 7 AND datediff( NOW(), `LastUpdated` ) < 14 ;', 'update');
}
function delObject14()
{
	sqlQuery('DELETE FROM `Object_DATA` WHERE datediff( NOW(), `LastUpdated` ) > 14 AND datediff( NOW(), `LastUpdated` ) < 28 ;', 'update');
}
function delObject28()
{
	sqlQuery('DELETE FROM `Object_DATA` WHERE datediff( NOW(), `LastUpdated` ) > 28 AND datediff( NOW(), `LastUpdated` ) < 365 ;', 'update');
}
function delObject365()
{
	sqlQuery('DELETE FROM `Object_DATA` WHERE datediff( NOW(), `LastUpdated` ) > 365 ;', 'update');
}
function delByObjectClass($VALUE = null)
{
	if (!empty($VALUE)) {
		return sqlQuery("DELETE FROM `Object_DATA` WHERE `Classname`='" . secure($VALUE) . "' AND `Instance`=" . (int)INSTANCE . ";", 'update');
	}
	return false;
}
		...
		if ($DATA !== false) {
			...
			if (isset($DATA['INVENTORY[0][0]']) && !empty($DATA['INVENTORY[0][1]'])) {
				if (is_array($DATA['INVENTORY[0][0]'])) {
					foreach ($DATA['INVENTORY[0][0]'] as $inv00) {
						if (!empty($inv00)) {
							$inventory[0][0][] = preg_replace('/[^a-zA-Z0-9_]/', null, $inv00);
						} else {
							$inventory[0][0] = [];
						}
					}
			...
			if (isset($DATA['INVENTORY[0][1]']) && !empty($DATA['INVENTORY[0][0]'])) {
				if (is_array($DATA['INVENTORY[0][1]'])) {
					foreach ($DATA['INVENTORY[0][1]'] as $inv01) {
						if (!empty($inv01)) {
							$inventory[0][1][] = (int)preg_replace('/[^0-9_]/', null, $inv01);
						} else {
							$inventory[0][1] = [];
						}
					}
			...
			if (isset($DATA['INVENTORY[1][0]']) && !empty($DATA['INVENTORY[1][1]'])) {
				if (is_array($DATA['INVENTORY[1][0]'])) {
					foreach ($DATA['INVENTORY[1][0]'] as $inv10) {
						if (!empty($inv10)) {
							$inventory[1][0][] = preg_replace('/[^a-zA-Z0-9_]/', null, $inv10);
						} else {
							$inventory[1][0] = [];
						}
					}
			...
			if (isset($DATA['INVENTORY[1][1]']) && !empty($DATA['INVENTORY[1][0]'])) {
				if (is_array($DATA['INVENTORY[1][1]'])) {
					foreach ($DATA['INVENTORY[1][1]'] as $inv11) {
						if (!empty($inv11)) {
							$inventory[1][1][] = (int)preg_replace('/[^0-9_]/', null, $inv11);
						} else {
							$inventory[1][1] = [];
						}
					}
			...
			if (isset($DATA['INVENTORY[2][0]']) && !empty($DATA['INVENTORY[2][1]'])) {
				if (is_array($DATA['INVENTORY[2][0]'])) {
					foreach ($DATA['INVENTORY[2][0]'] as $inv20) {
						if (!empty($inv20)) {
							$inventory[2][0][] = preg_replace('/[^a-zA-Z0-9_]/', null, $inv20);
						} else {
							$inventory[2][0] = [];
						}
					}
			...
			if (isset($DATA['INVENTORY[2][1]']) && !empty($DATA['INVENTORY[2][0]'])) {
				if (is_array($DATA['INVENTORY[2][1]'])) {
					foreach ($DATA['INVENTORY[2][1]'] as $inv21) {
						if (!empty($inv21)) {
							$inventory[2][1][] = (int)preg_replace('/[^0-9_]/', null, $inv21);
						} else {
							$inventory[2][1] = [];
						}
					}
...
function truncateTradersData()
{
	sqlQuery('TRUNCATE `Traders_DATA`;', 'update');
}
function deleteTraderData($VALUE = null)
{
	if (!empty($VALUE)) {
		if (is_array($VALUE)) {
			$LIMIT = count($VALUE);
			...
			return sqlQuery("DELETE FROM `Traders_DATA` WHERE `id` IN ( '" . implode("','", $IDS) . "' ) LIMIT " . $LIMIT . ";", 'update');
		}
		return false;
	}
}
...
function updateTraderData($IS = null, $ID = null, $VALUE = null)
{
	if (isset($IS, $ID, $VALUE) && is_numeric($ID)) {
		if ($IS === 'qty' && is_numeric($VALUE)) {
			$query = "UPDATE `Traders_DATA` SET `qty`=" . (int)$VALUE . " WHERE `id`=" . (int)$ID . " LIMIT 1;";
		}
		if ($IS === 'sell' && is_array($VALUE)) {
			$VALUE[0] = (int)$VALUE[0];
			$VALUE[2] = (int)$VALUE[2];
			$VALUE = json_encode($VALUE);
			$query = "UPDATE `Traders_DATA` SET `sell`='" . secure($VALUE) . "' WHERE `id`=" . (int)$ID . " LIMIT 1;";
		}
		if ($IS === 'buy' && is_array($VALUE)) {
			$VALUE[0] = (int)$VALUE[0];
			$VALUE[2] = (int)$VALUE[2];
			$VALUE = json_encode($VALUE);
			$query = "UPDATE `Traders_DATA` SET `buy`='" . secure($VALUE) . "' WHERE `id`=" . (int)$ID . " LIMIT 1;";
		}
		if (isset($query)) {
