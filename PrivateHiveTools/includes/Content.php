...
function ContentScripts($PAGE)
{
	...
	if ($row !== false) {
		...
		foreach ($row as $val) {
			if (isset($_GET[$val['key']])) {
				$src = isJson($val['phtScripts']);
				if ($src !== false || !empty($src)) {
					foreach ($src as $sc) {
						$s .= '<script src="scripts/' . $sc . '.js" charset="utf-8"></script>' . "\n";
					}
				}
			}
		}
...
function paginateLite($query, $limit = 10, $page = 'main', $and = '')
{
	$p = (empty($_GET[$page]) || !is_numeric($_GET[$page])) ? 1 : (int)$_GET[$page];
	if (isset($p)) {
		$rows = liteQuery($query, 'num');
		if ($rows <= $limit) {
			return ';';
		} else {
			echo "\n", '<script>', "\n", '//<![CDATA[', "\n", ' var curretPAGE=', $p, ';', "\n", ' var totalPAGE=', ceil($rows / $limit), ';', "\n", '//]]>', "\n", '</script>';
			return ' LIMIT ' . (int)(($p - 1) * $limit) . ',' . (int)$limit . ' ;';
		}
	}
	return ';';
}
function paginate($query, $limit = 10, $page = 'main', $and = '')
{
	$p = (empty($_GET[$page]) || !is_numeric($_GET[$page])) ? 1 : (int)$_GET[$page];
	if (isset($p)) {
		$rows = sqlQuery($query, 'num');
		if ($rows <= $limit) {
			return ';';
		} else {
			echo "\n", '<script>', "\n", '//<![CDATA[', "\n", ' var curretPAGE=', $p, ';', "\n", ' var totalPAGE=', ceil($rows / $limit), ';', "\n", '//]]>', "\n", '</script>';
