...
if (ini_get('register_globals') == 1) {
	...
	if (ini_get('register_globals') == 1) {
		die('<div style="background:#000;padding:5px;border:2px solid #8C0000;color:#fff;font-size:16px;">register_globals is set to <b>on</b>. This setting is dangerous.</div>');
	}
}
...
if (isset($CONF) && is_array($CONF)) {
	if (isset($GAMES[$CONF['GAME']])) {
		$CONF['GAME'] = $GAMES[$CONF['GAME']];
		require_once(DIR . '/includes/game_' . $CONF['GAME'] . '.php');
	}
...
if (defined('PHT_SETUP') && PHT_SETUP === true || !defined('PHT_SETUP')) {
